"""
Exporta todas as conversas com atividade em 01/04/2026 (horário de Brasília, UTC-3)
...
# ── Janela do dia 01/04/2026 em BRT (UTC-3) ──────────────────────────────────
BRT = timezone(timedelta(hours=-3))
DAY_START = datetime(2026, 4, 1, 0, 0, 0, tzinfo=BRT)   # 01/04 00:00 BRT
DAY_END   = datetime(2026, 4, 1, 23, 59, 59, tzinfo=BRT)  # 01/04 23:59 BRT

# startAfterDate precisa ser um instante ANTES da janela para a API incluir o início
CURSOR_START = DAY_START - timedelta(seconds=1)
...
def ts_to_iso(ts_ms):
    """Converte timestamp em ms (int) ou string ISO para data/hora em BRT."""
    if not ts_ms:
        return ""
    if isinstance(ts_ms, str):
        # já é ISO string — parseia e converte para BRT
        try:
...
def main():
    ...
    print(f"[export] Buscando conversas de {DAY_START.strftime('%d/%m/%Y')} BRT...")
    ...
    for conv in ghl.iter_conversations(start_after_date=CURSOR_START):
        ...
        # Conversas além do dia → podemos parar (ordem ascendente)
        if dt and dt > DAY_END:
            skipped_after += 1
            if skipped_after >= 3:
                break
            continue
    ...
    output_path = os.path.join(
        os.path.dirname(__file__), "conversas_01_04_2026.xlsx"
    )
    wb.save(output_path)
